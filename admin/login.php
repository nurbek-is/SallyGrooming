    ...
    try {
     // Prepare and execute the $qLogin query
      ...
      if (!$row = $stmt->fetch()) {
        ...
        $loginFailed = true;
        $failureMessage = nl2br(LOGIN_FAILED);
      } elseif ( !$row['registration_confirmed']) {
        // user never confirmed registration
        $loginFailed = true;
        $failureMessage = nl2br(REGISTRATION_UNCONFIRMED);
      ...
      logError($e);
      $loginFailed = true;
      $failureMessage = nl2br(GENERIC_ERROR);
    }
  }
?>
...
<main class="small">
  ...
  <?php
    if (isset($loginFailed)) {
      echo "<article class='error'>$failureMessage</article>";
    }
